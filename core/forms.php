...
if ( ! function_exists( 'antreas_form_collection' ) ) {
	function antreas_form_collection( $name, $value, $list, $args = null ) {
		...
		foreach ( $value as $current_key => $current_value ) {
			...
			$output .= '<td>';
			$output .= '<a href="#" tabindex="-1" class="collection-remove-row">' . __( 'Remove', 'antreas' ) . '</a>';
			$output .= '</td>';
			$output .= '</tr>';
		}
		$output .= '<tr>';
		$output .= '<td>';
		$output .= '<a href="#" class="button collection-add-row">' . __( 'Add Row', 'antreas' ) . '</a>';
		$output .= '</td>';
		$output .= '</tr>';
		$output .= '</table>';
...
if ( ! function_exists( 'antreas_form_color' ) ) {
	function antreas_form_color( $name, $value, $args = null ) {
		...
		$output  = '<div id="' . $name . '_wrap">';
		$output .= '<input type="text" class="cpo-color-selector" value="' . esc_attr( $value ) . '" name="' . $name . '" id="' . $name . '"' . $field_placeholder . ' maxlength="7"/>';
...
if ( ! function_exists( 'antreas_form_upload' ) ) {
	function antreas_form_upload( $name, $value, $args = null, $post = null ) {
		...
		$output .= '<input class="upload_button" type="button" value="' . __( 'Upload', 'antreas' ) . '" name="' . $name . '" id="' . $name . '-button"/>';
...
if ( ! function_exists( 'antreas_form_font' ) ) {
	function antreas_form_font( $name, $value, $list, $args = null ) {
		...
		$output .= '</div>';
		$output .= '<div type="text" class="font_preview" id="' . $name . '-preview" style="font-family:\'' . $font_name . '\'; font-weight:' . $weight . ';">' . __( 'This is a font preview image', 'antreas' ) . '</div>';
		return $output;
	}
}
...
if ( ! function_exists( 'antreas_form_range' ) ) {
	function antreas_form_range( $name, $value, $args = null ) {
		...
		$output = '<div class="cpotheme-range-control">';
			$output .= '<div class="slider" slider-min-value="' . esc_attr( $args['attrs']['min'] ) . '" slider-max-value="' . esc_attr( $args['attrs']['max'] ) . '" slider-step-value="' . esc_attr( $args['attrs']['step'] ) . '"></div>';
			$output .= '<input type="number" step="' . esc_attr( $args['attrs']['step'] ) . '" id="' . $name . '" name="' . $name . '" value="' . esc_attr( $value ) . '" />';
		$output .= '</div>';
