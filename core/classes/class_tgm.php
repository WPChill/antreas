		...
		public function notices() {
			...
			if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) ) {
				...
				if ( ! empty( $message ) ) {
					...
					$notice    = '<span class="cpotheme-notice">';
					$notice   .= '<a class="cpothemes-notice-dismiss" style="font-weight:normal;" href="' . add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . __( 'Dismiss This Notice', 'antreas' ) . '</a>';
					$notice   .= '<img class="cpotheme-wizard-image" src="' . $core_path . 'images/ct-icon.png">';
					$notice   .= '<span class="cpotheme-wizard-title" style="display:block; margin:0 0 5px; font-size:1.3em;">' . sprintf( __( 'Install Recommended Plugins', 'antreas' ), CPOTHEME_NAME ) . '</span>';
					...
					foreach ( $message as $type => $plugin_groups ) {
						...
						$imploded = implode( ', ', $plugin_groups );

						//$rendered .= $imploded;
						$rendered .= sprintf( translate_nooped_plural( $this->strings[ $type ], $count, $this->domain ), $imploded, $count ); // All messages now stored
					}
					...
					if ( $action_links ) {
						$rendered .= '<p style="margin-left:57px;">' . implode( ' | ', $action_links ) . '</p>';
					}
					...
					if ( isset( $this->strings['nag_type'] ) ) {
						add_settings_error( 'tgmpa', 'tgmpa', $notice, sanitize_html_class( strtolower( $this->strings['nag_type'] ), 'updated' ) );
					} else {
						add_settings_error( 'tgmpa', 'tgmpa', $notice, 'updated' );
					}
				}
			}
