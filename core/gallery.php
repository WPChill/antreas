...
function cpotheme_metabox_gallery( $post ) {
	...
	if ( $attachments ) {
		foreach ( $attachments as $attachment_id ) {
			if ( trim( $attachment_id ) != '' ) {
				...
				$output .= '<a href="#" class="cpotheme-gallery-remove delete tips" data-tip="' . __( 'Remove', 'antreas' ) . '">' . __( 'Remove', 'antreas' ) . '</a>';
				$output .= '</li>';
			}
		}
	}
	...
	$output .= '<a href="#" data-choose="' . __( 'Add Images', 'antreas' ) . '" data-update="' . __( 'Add to gallery', 'antreas' ) . '" data-delete="' . __( 'Delete image', 'antreas' ) . '" data-text="' . __( 'Delete', 'antreas' ) . '">';
	$output .= __( 'Add Images', 'antreas' );
	$output .= '</a>';
	$output .= '</p>';
...
function cpotheme_gallery_display( $imagelist ) {
	...
	foreach ( $imagelist as $image ) :
		...
		$gallery  .= '<a href="' . admin_url( '/post.php?post=' . $image->ID . '&action=edit&image-editor' ) . '" title="' . __( 'Edit This Image', 'antreas' ) . '" target="_blank">';
