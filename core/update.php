...
function cpotheme_license() {
	add_options_page( __( 'CPOThemes License', 'antreas' ), __( 'CPOThemes License', 'antreas' ), 'manage_options', 'cpotheme_license', 'cpotheme_license_page' );
}

//Render settings page
function cpotheme_license_page() {
	echo '<div class="wrap">';
	echo '<h2>' . __( 'CPOThemes License', 'antreas' ) . '</h2>';
	//settings_errors();
	echo '<form method="post" action="options.php">';
	settings_fields( 'license_' . CPOTHEME_ID );
	do_settings_sections( 'license_' . CPOTHEME_ID );
	submit_button( __( 'Activate Your License', 'antreas' ) );
	echo '</form>';
	echo '</div>';
}
...
function cpotheme_license_section( $args ) {
	...
	if ( $current_license != false && $current_license != '' ) {
		if ( $current_license_status == 'invalid' ) {
			...
			echo __( 'The theme license you entered is invalid.', 'antreas' );
			echo '</div>';
		} elseif ( $current_license_status == 'valid' ) {
			echo '<div class="cpotheme-message cpotheme-message-success">';
			echo __( 'Your license key has been activated.', 'antreas' );
			echo '</div>';
		}
	}
	printf( __( 'Please add your CPOThemes license key in order to get automatic theme updates for %s directly from your WordPress site.', 'antreas' ), CPOTHEME_NAME );
	echo '<br>';
	echo '<a target="_blank" href="//www.cpothemes.com/dashboard/purchase-history">' . __( 'Obtain the license key at CPOThemes', 'antreas' ) . '</a>';
...
function cpotheme_license_metadata() {
	...
	$data['license'] = array(
		'label'       => __( 'License Key', 'antreas' ),
		'description' => __( 'Add your theme license key to enable automatic updates from the dashboard.', 'antreas' ) . '<br><a href="http://www.cpothemes.com/dashboard/purchase-history" target="_blank">' . __( 'Get Your License Key.', 'antreas' ) . '</a>',
...
if ( ! function_exists( 'cpotheme_license_setup' ) ) {
	...
	function cpotheme_license_setup() {
		...
		$updater = new CPOTheme_Core_Updater(
			array(
...
function cpotheme_license_activate( $new_license ) {
	...
	if ( $new_license != '' && ( $license_status != 'valid' || $new_license != $current_license ) ) {
		...
		$response = wp_remote_get(
			add_query_arg( $args, CPOCORE_STORE ), array(
				'timeout'   => 15,
				'sslverify' => false,
			)
		);
		...
		if ( current_user_can( 'manage_options' ) ) {
			if ( $current_license_dismissed != 'dismissed' ) {
				...
				if ( $current_license_status != 'valid' ) {
					...
					echo '<a href="' . add_query_arg( 'ctdismiss', CPOTHEME_ID . '_license' ) . '" class="cpotheme-notice-dismiss">' . __( 'Dismiss This Notice', 'antreas' ) . '</a>';
					if ( $current_license_status == 'invalid' && $current_license != '' ) {
						echo '<span style="color:red;">';
						echo __( 'The theme license you entered is invalid!', 'antreas' );
						echo '<br></span>';
					}
					echo __( 'Please add your CPOThemes license key in order to get automatic theme updates from your dashboard.', 'antreas' );
					echo '<br>';
					echo '<b><a href="options-general.php?page=cpotheme_license">' . __( 'Enter License Key', 'antreas' ) . '</a></b>';
					echo ' | ';
					echo '<a target="_blank" href="//www.cpothemes.com/dashboard/purchase-history">' . __( 'Obtain the license key at CPOThemes', 'antreas' ) . '</a>';
